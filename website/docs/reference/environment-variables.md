...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly when no explicit/scoped board is active. The dispatcher injects this into worker subprocess env so profile workers converge on the dispatcher's board |
| `HERMES_KANBAN_WORKSPACES_ROOT` | Pin the kanban workspaces root directly when no explicit/scoped board is active. The dispatcher injects this into worker subprocess env |
